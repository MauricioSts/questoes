...
const colorModeToFloat = (mode: "molten" | "ember" | "frost") =>
...
export function MoltenMetal({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Se WebGL 2 não for suportado no ambiente
      return;
    }
    ...
    let handlePointerMove: ((e: PointerEvent | MouseEvent) => void) | null = null;
    let handlePointerLeave: (() => void) | null = null;

    if (mouseInteraction) {
      handlePointerMove = (e: PointerEvent | MouseEvent) => {
        const rect = container.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    const ubg = u.uBackgroundColor.value;
